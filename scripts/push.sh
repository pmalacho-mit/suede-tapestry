#!/usr/bin/env bash
#
# Run `git subrepo push` for every folder inside the .suede directory.
#
set -euo pipefail
...
for folder in "${folders[@]}"; do
    ...
    path="${folder%/}"
    echo "==> git subrepo push ${path}"
    if ! git subrepo push "$path"; then
        failed+=( "$path" )
    fi
    echo
done
...
if [[ ${#failed[@]} -gt 0 ]]; then
    echo "Failed to push ${#failed[@]} subrepo(s):" >&2
    printf '  - %s\n' "${failed[@]}" >&2
    exit 1
fi
